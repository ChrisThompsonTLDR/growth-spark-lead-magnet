...
const HeroSection = ({ scrollToForm }: HeroSectionProps) => {
  return (
    <div className="relative w-full h-[90vh] min-h-[600px] max-h-[900px] flex items-center justify-center overflow-hidden px-4 md:px-8">
      {/* Background gradient */}
      <div className="absolute inset-0 bg-gradient-to-b from-growth-50 to-white z-0"></div>
      
      {/* Decorative shapes */}
      <div className="absolute top-20 left-10 w-64 h-64 rounded-full bg-growth-100 blur-3xl opacity-40 z-0 animate-pulse-light"></div>
      <div className="absolute bottom-20 right-10 w-80 h-80 rounded-full bg-growth-200 blur-3xl opacity-30 z-0 animate-pulse-light" style={{ animationDelay: '1s' }}></div>
